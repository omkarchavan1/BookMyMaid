...
export default class Maidbookinginfo extends React.Component {
  ...
  componentDidMount = (maidId, categoryname) => {
    ...
    fetch("http://localhost:8092/categoryId?maidId=" + maidId + "&categoryname=" + categoryname)
  ...
  getMaids = (maidId) => {
    fetch("http://localhost:8092/getmaids?maid_id=" + maidId)
      .then(resp => resp.json())//.then(data => alert(JSON.stringify(data)))
  ...
  changeStatus = () => {
    ...
    fetch("http://localhost:8092/updatemaid", requestOption)
      .then(res => {
        if (res.status == 200) { }
      })
  }
  ...
  book = (event) => {
    ...
    fetch('http://localhost:8092/savebooking', requestOption)
      .then(res => {
        if (res.status == 200) {
