...
class AdminMaid extends React.Component {
  ...
  componentDidMount = () => {
    fetch('http://localhost:8092/maid/maidlist')
      .then(resp => resp.json())
      .then(data => {
...
componentWillUnmount = ()=>{
  fetch('http://localhost:8092/maid/maiddelete/{id}')
  .then(resp => resp.json())
      .then(data => {
        this.setState({ maids: data }); console.log(this.state.maids)
      })
  ...
  delete = (maids) => (event) => {
    ...
    fetch('http://localhost:8092/maid/maiddelete/' + maids.maidId, requestOption)
      .then(res => {
        if (res.status === 200) {
